...
public class TaurusBulletSystem5 : BossShooter<BossBullet>
{
    ...
    List<BossBullet> bullets = new(WaveCount * BranchCount * BulletCount);
        ...
        for (int i = 0; i < WaveCount; i++)
        {
            ...
            for (int ii = 0; ii < BranchCount; ii++)
            {
                for (int iii = 0; iii < BulletCount; iii++)
                {
                    ...
                    bullets[b].StartCoroutine(bullets[b].TransformRotateAround(EnemyMovementBehaviour.bossSpawnPosition, Mathf.Infinity, r * BulletRotationSpeed));
                }
            }
        }
