...
public class TaurusLaser60 : Laser
{
    ...
    {
        base.Awake();
        originalParent = FindObjectOfType<BossLaserPool>().transform;
    }
    ...
    protected override void HandleCollision(Collider2D coll)
    {
        ...
        if (NumCollisions > 1)
        {
            for (int i = 0; i < NumCollisions; i++)
            {
                ...
                if (collisionResults[i].TryGetComponent(out BossBullet _) && sqrDistance < closestCollisionDistance)
                {
        ...
        if (coll.TryGetComponent(out BossBullet _))
        {
